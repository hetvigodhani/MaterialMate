...
if (strlen($_SESSION['ocasaid']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $fileExtension = pathinfo($file1, PATHINFO_EXTENSION); // Get the file extension

        $allowedExtensions = array("pdf");

        if (in_array(strtolower($fileExtension), $allowedExtensions)) {
            $file1 = md5($file1) . time() . '.' . $fileExtension;
            move_uploaded_file($_FILES["file1"]["tmp_name"], "folder1/" . $file1);

            $sql = "update tblcourse set File=:file1 where ID=:eid";
            $query = $dbh->prepare($sql);
            $query->bindParam(':file1', $file1, PDO::PARAM_STR);
            $query->bindParam(':eid', $eid, PDO::PARAM_STR);
            $query->execute();

            echo '<script>alert("Course doc file has been updated")</script>';
        } else {
...
<html lang="en">
<head>
    <title>OCMMS || Update Course</title>
                ...
                <div class="row g-4">
                    <div class="col-sm-12 col-xl-6">
                        <div class="bg-light rounded h-100 p-4">
                            <h6 class="mb-4">Update Course File First</h6>
                            <form method="post">
                                <?php
                                $eid=$_GET['editid'];
